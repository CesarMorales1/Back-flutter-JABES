...
Admin.getRelacion = () =>
{
    const sql = `SELECT users.email, id_rol, roles.name
    FROM public.user_has_roles
    INNER JOIN public.users
    ON public.user_has_roles.id_user = public.users.id
    INNER JOIN public.roles
    ON public.roles.id = public.user_has_roles.id_rol;`
